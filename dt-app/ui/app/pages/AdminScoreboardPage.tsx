...
export const AdminScoreboardPage: React.FC = () => {
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      id: 'timeRemaining',
      header: 'Time Left',
      accessor: 'timeRemaining' as const,
      width: 120,
      columnType: 'text' as const,
  ...
  const tableData = useMemo(() => {
    return topScores.map((result, index) => ({
      id: `${result.name}-${result.email}-${result.score}-${index}`,
      rank: index + 1,
      name: result.name,
      email: result.email,
          ...
          <Flex justifyContent="space-between" alignItems="center" style={{
            ...
            <Button 
              onClick={() => {
                // Create CSV content
                const csvContent = [
                  'Rank,Name,Email,Time Left,Score',
                  ...tableData.map((row, index) => 
                    `${getRankDisplay(index)},"${row.name}","${row.email}",${formatTimeRemaining(row.timeRemaining)},${row.score}`
                  )
                ].join('\n');
                ...
                link.href = url;
                link.download = 'bug-busters-admin-leaderboard.csv';
                link.click();
